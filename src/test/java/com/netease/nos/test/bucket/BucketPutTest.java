...
public class BucketPutTest {
	...
	public void before(){
		...
		client2 = new NosClient(credentials2,clientConfiguration);
	}
	...
	public void testPutBucketWithoutRegion(CreateBucketRequest createBucketRequest){
		...
		Bucket bucket = client.createBucket(createBucketRequest.getBucketName(),Region.CN_EAST_P1,true);
		Assert.assertNotNull(bucket);
		Assert.assertEquals(bucket.getName(),createBucketRequest.getBucketName());
		Assert.assertTrue(client.doesBucketExist(createBucketRequest.getBucketName()));
	}
	...
	public void testPutBucket(){
		Credentials credentials = new BasicCredentials("2d98d96564f64b038eb69d81fa84f7ad","1f29c502cc5d4227b40f3ffcd885ebf6");
		ClientConfiguration clientConfiguration = new ClientConfiguration().withProtocol(Protocol.HTTPS);
		clientConfiguration.setIsSubdomain(true);
		NosClient nosClient = new NosClient(credentials,clientConfiguration);
		nosClient.setEndpoint("nos-hz.163yun.com");
        nosClient.deleteBucket("testbba");
	}
	...
	public void testPutObject(){
		...
		nosClient.setEndpoint("nos-hz.163yun.com");
        nosClient.deleteObject("testbba","test");
