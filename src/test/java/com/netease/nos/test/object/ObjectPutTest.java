...
public class ObjectPutTest {
	...
	public void before() {
		...
		client2 = new NosClient(credentials2,configuration);
		CreateBucketRequest createBucketRequest2 = new CreateBucketRequest(
	            DataProvidedForPutObject.bucketownedbyothers, TestHostConfig.region, false);
	...
	public void after(){
		try{
			...
			Clear.clear(client, DataProvidedForPutObject.bucketownedbyothers);
//			Clear.clearAllVersions(client, DataProvidedForPutObject.bucketfortestputobject);
	...
	@Test
	public void testPutObjectWithoutName() {
		File file=DataProvidedForPutObject.createSampleFile();
		file.deleteOnExit();
		try {
			client.putObject(DataProvidedForPutObject.bucketfortestputobject,file);
			NOSObject object = client.getObject(DataProvidedForPutObject.bucketfortestputobject, file.getName());
			String expectedMD5 = TestHelper.getMD5(file.getPath());
			String actualMd5 = object.getObjectMetadata().getETag();
			Assert.assertEquals(actualMd5.toUpperCase(), expectedMD5);
		} catch (Exception e) {
			Assert.fail(e.getMessage());
		}
		ObjectMetadata meta = client.getObjectMetadata(DataProvidedForPutObject.bucketfortestputobject, file.getName());
