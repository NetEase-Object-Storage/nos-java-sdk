...
public class ObjectCopyAndMoveTest {
	...
	public void before(){
		...
		CreateBucketRequest createBucketRequest = new CreateBucketRequest(
				DataProvidedForCopyObject.sourcebucketfortestcopyobject,TestHostConfig.region, false);
		...
		createBucketRequest = new CreateBucketRequest(
				DataProvidedForCopyObject.descbucketfortestcopyobject,TestHostConfig.region, false);
		if (!client.doesBucketExist(createBucketRequest.getBucketName())) {
			Bucket bucket = client.createBucket(createBucketRequest);
			Assert.assertNotNull(bucket);
			Assert.assertEquals(bucket.getName(), createBucketRequest.getBucketName());
		...
		if (!client.doesObjectExist(putObjectRequest.getBucketName(), putObjectRequest.getKey(), null)) {
		...
		CreateBucketRequest createBucketRequest2 = new CreateBucketRequest(
				DataProvidedForCopyObject.othersbucketfortestcopyobject, TestHostConfig.region, false);
		...
		if (!client2.doesObjectExist(putObjectRequest2.getBucketName(),putObjectRequest2.getKey(), null)) {
	...
	public void testCopyObjectNormal(CopyObjectRequest copyObjectRequest) {
		...
		Assert.assertTrue(client.doesObjectExist(copyObjectRequest.getDestinationBucketName(),
				copyObjectRequest.getDestinationKey(), null));
		...
		Assert.assertTrue(client.doesObjectExist(copyObjectRequest.getDestinationBucketName(), "objectcopyfromsamebucket", null));
		...
		Assert.assertTrue(client.doesObjectExist(copyObjectRequest.getSourceBucketName(), "moveobject", null));
		...
		Assert.assertTrue(client.doesObjectExist(copyObjectRequest.getSourceBucketName(), "objectmovefromsamebucket", null));
	}
