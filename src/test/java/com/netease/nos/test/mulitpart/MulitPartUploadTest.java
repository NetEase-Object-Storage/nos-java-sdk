...
public class MulitPartUploadTest {
	...
	public void before(){
		...
		if (!client.doesBucketExist(bucketName)) {
			CreateBucketRequest createBucketRequest = new CreateBucketRequest(bucketName, TestHostConfig.region, false);
	...
	**/
	...
	public void testMetadataUpload(){
		String objectKey = "objectKey5";
		if(client.doesObjectExist(bucketName,objectKey,null))
			client.deleteObject(bucketName,objectKey);
		InitiateMultipartUploadRequest initiateMultipartUploadRequest = new InitiateMultipartUploadRequest(bucketName,
				objectKey);
		ObjectMetadata uploadMetadata=new ObjectMetadata();
		uploadMetadata.setContentType("application/zip");
        Map<String, String> userMeta = new HashMap<String, String>();
        userMeta.put("normal-meta","test normal metadata");
        uploadMetadata.setUserMetadata(userMeta);
		initiateMultipartUploadRequest.setObjectMetadata(uploadMetadata);
		initiateMultipartUploadRequest.addSpecialHeader("x-nos-meta-header-meta","test header metadata");
        InitiateMultipartUploadResult result = client.initiateMultipartUpload(initiateMultipartUploadRequest);
		Assert.assertEquals(objectKey, result.getKey());
		String uploadId = result.getUploadId();
		...
		for (PartSummary ps : Parts.getParts()) {
        ...
        Assert.assertEquals(objectMetadata.getContentType(),"application/zip");
        Assert.assertEquals(objectMetadata.getUserMetadata().get("normal-meta"),"test normal metadata");
        Assert.assertEquals(objectMetadata.getUserMetadata().get("header-meta"),"test header metadata");
	}
