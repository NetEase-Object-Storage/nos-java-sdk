...
public class Clear {
	...
	//删除桶内的所有文件
	public static void clearObjects(Nos client, String bucketName) {
		if (!client.doesBucketExist(bucketName))
			return;
		List<String> keys = new ArrayList<String>();
		listAllObjects(client, bucketName,"",keys);
	...
	/*public static void clearAllVersions(Nos client, String bucketName) {
		if (!client.doesBucketExist(bucketName))
			return;

		String versionStatus = client.getBucketVersioningConfiguration(bucketName).getStatus();
		if (versionStatus.equalsIgnoreCase(BucketVersioningConfiguration.ENABLED)) {
			//ObjectListing objectListing = client.listObjects(bucketName);
			List<String> keys = new ArrayList<String>();
			//List<NOSObjectSummary> sum = objectListing.getObjectSummaries();
			listAllObjects(client, bucketName,"",keys);
			for (String key : keys) {
				GetObjectVersionsRequest getObjectVersionsRequest = new GetObjectVersionsRequest(bucketName,
						key);
				GetObjectVersionsResult GetObjectVersionsResult =null;
				try{
					GetObjectVersionsResult = client.getObjectVersions(getObjectVersionsRequest);
				}catch(Exception e){
					continue;
				}
				
				if(GetObjectVersionsResult != null){
					List<NOSVersionSummary> NOSVersionSummarys = GetObjectVersionsResult.getVersionSummary();
					for (NOSVersionSummary nos : NOSVersionSummarys) {
						client.deleteObject(bucketName, nos.getKey(), nos.getVersionId());
					}
					client.deleteObject(bucketName, key);
				}
			...
			VersionListing hisversions = client.listVersions(bucketName);
			List<NOSVersionSummary>  versionSummaries = hisversions.getVersionSummaries();
			if(versionSummaries != null){
				for(NOSVersionSummary ver : versionSummaries){
					client.deleteObject(bucketName, ver.getKey(), ver.getVersionId());
				}
			}
